"""
ShutterstockAnalyzer v2.0.0
AI Metadata Generator for Stock Photography — entry point.
"""
...
class App(ctk.CTk):
    """Main application window — wires the active pages to the v2 API."""
    ...
    def __init__(self):
        ...
        # Window icon (top-left corner). resource_path makes this work both
        # in source tree and in a PyInstaller --onefile bundle (sys._MEIPASS).
        ...
        # Page references and selection state
        self.ai_control_page: AIControlPage | None = None
        self.scan_page: ScanPage | None = None
    ...
    def _create_ai_control_tab(self):
        ...
        settings = self.api.database.get_all_settings() if self.api else {}
        try:
            self.ai_control_page = AIControlPage(
                tab, settings=settings, on_status_change=self._on_ai_status_change
            )
    ...
    def _start_ai_processing(self):
        ...
        def process():
            try:
                ...
                self.after(0, lambda r=result: self._on_process_complete(r))

            except Exception as e:
                logger.error(f"Processing error: {e}")
                # Capture e as default arg so the lambda survives the except
                # block exit (Python 3 deletes the exception variable on exit).
...
def main():
    """Application entry point: AppUserModelID, splash, theme, App.mainloop."""
    ...
    splash = SplashScreen("ShutterstockAnalyzer", "2.0.0", width=450, height=220)
    splash.show()
    splash.update_progress(50, "Configuring theme...")
    splash.root.update()

    # Theme must be set before any CTk widget is created.
    ...
    logger.info("=" * 50)
    logger.info("ShutterstockAnalyzer v2.0.0")
    logger.info("=" * 50)
